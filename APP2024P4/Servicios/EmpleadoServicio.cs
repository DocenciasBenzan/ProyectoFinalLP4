...
{
	Task<Result> ActualizarEmpleado(EmpleadoRequest request);
...
public class EmpleadoServicio(IApplicationDbContext context) : IEmpleadoServicio
{
	...
	public async Task<Result> ActualizarEmpleado(EmpleadoRequest request)
	{
		try
		{
			var empleado = context.Empleados.FirstOrDefault(x => x.Id == request.Id);
			if (empleado == null)
			{
				return Result.Failure("Empleado no encontrado:");
			}
			if (empleado.Actualizar(request))
			{
				await context.SaveChangesAsync().ConfigureAwait(false);
				return Result.Success();
			}
